	...
	int nCmdShow)
{
	...
	EXIT_ON_ERROR(hr)
		...
		//InitCommonControls();//空函数，不做任何事，下面创建对话框
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		hr = g_pEndptVol->GetMute(&bMute);//得到静音状态
		ERROR_CANCEL(hr)
			SendDlgItemMessage(hDlg, IDC_CHECK_MUTE, BM_SETCHECK,
			bMute ? BST_CHECKED : BST_UNCHECKED, 0);
		hr = g_pEndptVol->GetMasterVolumeLevelScalar(&fVolume);//得到主音量的大小浮点数（0.0~1.0）
		ERROR_CANCEL(hr)
			nVolume = (int)(MAX_VOL*fVolume + 0.5);//将音量转化为整数（0到100） MAX_VOL为100
	...
	case WM_HSCROLL:
		...
		case SB_LEFT:
			...
			hr = g_pEndptVol->SetMute(FALSE, &g_guidMyContext);//设置静音
			ERROR_CANCEL(hr)
				nVolume = SendDlgItemMessage(hDlg, IDC_SLIDER_VOLUME, TBM_GETPOS, 0, 0);
			fVolume = (float)nVolume / MAX_VOL;//设置音量
